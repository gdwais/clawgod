...
const fs = require('fs');
const { header, check, bold, cyan, dim, green, red, yellow } = require('../wizard/ui');
const { createRL, select, confirm } = require('../wizard/prompts');
const { AGENT_DEFINITIONS } = require('../utils/config');
const { generateSingleAgent } = require('../utils/templates');

const DEFAULT_OPENCLAW_DIR = path.join(os.homedir(), '.openclaw');

function parseArgs(argv) {
  const args = { type: null, dir: null };
...
module.exports = async function addAgent(argv) {
    ...
    try {
      ...
      if (!args.dir) {
        ...
        dir = await new Promise((resolve) => {
          ...
          rl.question(`  ${cyan('?')} Instance directory ${dim(`[${defaultDisplay}]`)}: `, (answer) => {
