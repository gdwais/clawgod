...
const { header, bold, cyan, dim, yellow, green } = require('../wizard/ui');
const { createRL } = require('../wizard/prompts');

function parseArgs(argv) {
  const args = { host: null, user: null, dir: './output' };
    ...
    try {
      ...
      if (!host) {
        host = await new Promise((resolve) => {
          rl.question(`  ${cyan('?')} Remote host: `, (a) => resolve(a.trim()));
        });
      ...
      if (!user) {
        ...
        user = await new Promise((resolve) => {
          rl.question(`  ${cyan('?')} SSH user ${dim(`[${defaultUser}]`)}: `, (a) => resolve(a.trim() || defaultUser));
        });
      }
    } finally {
      rl.close();
